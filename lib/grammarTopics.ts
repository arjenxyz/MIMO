...
type SeedItem = Omit<GrammarItem, "id" | "topic_id" | "sort_order"> & { sort_order?: number };

/** Drill packs keyed by curriculum slug (expand over time). */
const DRILL_ITEMS: Record<string, SeedItem[]> = {
  "in-on-at": [
...
export function getDemoGrammarTopics(): GrammarTopic[] {
  return GRAMMAR_CURRICULUM.map((topic, i) => ({
    ...
    item_count: DRILL_ITEMS[topic.slug]?.length ?? 0,
  }));
}

export function getDemoGrammarTopicBySlug(slug: string): GrammarTopicDetail | null {
  const idx = GRAMMAR_CURRICULUM.findIndex((t) => t.slug === slug);
  if (idx === -1) return null;
  const topic = GRAMMAR_CURRICULUM[idx];
  const pack = DRILL_ITEMS[slug] ?? [];
  return {
    id: idx + 1,
    slug: topic.slug,
    title: topic.title,
    ...
    difficulty: topic.difficulty,
    category: topic.category,
    item_count: pack.length,
    items: pack.map((item, j) => ({
      id: (idx + 1) * 1000 + j + 1,
      topic_id: idx + 1,
      question: item.question,
      correct_answer: item.correct_answer,
...
export function difficultyLabel(level: number) {
  if (level >= 1 && level <= 5) return CEFR_LABEL[level as CefrBand];
  return "A1";
}

export { getCurriculumTopic, GRAMMAR_CURRICULUM, CEFR_SECTION_TITLE };
