...
export function GrammarCatalog({ topics }: { topics: GrammarTopic[] }) {
  ...
  const filtered = useMemo(
    () => topics.filter((t) => matchesQuery(t, query)),
    [topics, query]
  );
  ...
  return (
    <>
        ...
        <div className="min-w-0 flex-1">
          ...
          <label className="mt-3 block">
            ...
            <input
              ...
              placeholder="Konu ara… (ör. present, if, passive)"
              autoComplete="off"
              className="w-full rounded-xl border border-mimo-border bg-mimo-card px-3.5 py-2.5 text-sm font-semibold text-mimo-fg outline-none placeholder:text-mimo-muted focus:border-[#ce82ff]"
            />
          </label>
        </div>
        <Link
          href="/"
          className="mt-1 shrink-0 rounded-full px-3 py-1.5 text-sm font-bold text-mimo-muted transition hover:bg-mimo-surface hover:text-mimo-fg"
        >
          Ana sayfa
        </Link>
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="rounded-2xl border border-mimo-border bg-mimo-card px-4 py-8 text-center text-sm font-bold text-mimo-muted">
          “{query}” için konu bulunamadı.
        </p>
      ) : (
            ...
            return (
              <section key={band} aria-labelledby={`cefr-${band}`}>
                <div className="mb-3 flex items-baseline justify-between gap-2">
                  <h2
                    id={`cefr-${band}`}
                    className="text-sm font-black uppercase tracking-[0.14em] text-mimo-title"
                  >
